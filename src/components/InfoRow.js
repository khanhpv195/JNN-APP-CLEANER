...
const InfoRow = ({ label, value, badge = false, col = false, color = "bg-green-500" }) => {
  return (
    <View className={`justify-between items-start py-3 gap-2 ${col ? 'flex-col' : 'flex-row'}`}>
      <Text style={{/* was: className="text-gray-600" */}}>{label}</Text>
      {badge ? (
        <View className={`${color} px-3 py-1 rounded-full`}>
          <Text style={{/* was: className="text-white text-sm" */}}>{value}</Text>
        </View>
      ) : (
        <Text style={{/* was: className="text-gray-900 font-semibold" */}}>{value}</Text>
      )}
    </View>
  )
}

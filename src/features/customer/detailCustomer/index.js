...
export default function DetailCustomerScreen() {
  ...
  const leadId = routeLead?._id

  const { lead, isLoading, error, refetch } = useGetLeadDetail(leadId);
  ...
  return (
    <View className="flex-1 bg-white p-4">
      ...
      <View className="flex-row justify-between items-center mb-4">
        <Text className="text-xl font-bold">{customerData?.name || 'Customer Details'}</Text>
        <View className="flex-row space-x-3">
          <TouchableOpacity
            onPress={() => NavigationService.navigate('form_customer', { lead: customerData, edit: true })}
      ...
      <View className="flex-1">
        <ScrollView className="flex-1">
          ...
          <View className="px-4 border border-neutral-200 rounded-2xl divide-y divide-neutral-200 mb-5">
            ...
            <InfoRow label="Assigned to" value={customerData?.assignedToUsername} />

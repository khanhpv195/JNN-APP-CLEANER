...
export default function FormLeadComponent() {
  ...
  useLayoutEffect(() => {
    const title = edit ? 'Edit Lead' : 'Create Lead'
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (edit) {
        ...
        navigation.navigate('lead_detail', {
          lead: {
            ...lead,
            ...data,
            id: lead?.id || lead?._id,
        ...
        navigation.navigate('lead_list', { refresh: true });
      }
    } catch (error) {
            ...
            render={({ field: { value } }) => (
              <>
                ...
                >
                  <Text className={value ? "text-gray-700" : "text-gray-400"}>
                    {value
                      ? LEAD_STATUSES.find(option => option.value === value)?.label
                      : "Select status"}
                  </Text>
                  <ChevronDownIcon size={20} color="#9CA3AF" />
                </TouchableOpacity>
                ...
                {showStatusOptions && (
                  <View className="w-full border border-gray-200 rounded-lg mt-1 bg-white overflow-hidden">
                    {LEAD_STATUSES.map((option, index) => (
                      <TouchableOpacity
                        key={option}
                        onPress={() => handleSelectStatus(option)}
                        className={`p-3 ${index < LEAD_STATUSES.length - 1 ? 'border-b border-gray-100' : ''}`}
                      >
                        <Text className="text-gray-700">{option.label}</Text>
                      </TouchableOpacity>
                    ))}
